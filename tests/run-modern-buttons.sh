#!/bin/sh
# Check hover, focus, pressed and toggle borders in real toolbar/dialog buttons.
set -eu
cd "$(dirname "$0")/.."
test_dir=$(mktemp -d "${TMPDIR:-/tmp}/emelfm2-modern-buttons.XXXXXX")
...
for theme in os.environ.get('E2_MODERN_TEST_THEMES', 'Adwaita,Adwaita-dark').split(','):
    ...
    for mode in ('false', 'true'):
        ...
        env = dict(os.environ, LD_PRELOAD=str(root/'test.so'), GTK_THEME=theme,
                   GTK2_RC_FILES=str(rc), E2_BUTTON_TEST_CASE=case)
        ...
        assert 'button hover, focus, press, toggle, disabled state and activation passed' in log, log
